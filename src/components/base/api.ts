...
export class Api {
    ...
    protected handleResponse(response: Response): Promise<object> {
        if (response.ok) 
            ...
            return response.json() // Иначе, обрабатываем JSON ответа, который может содержать ошибку, или возвращаем текст ошибки по умолчанию
    ...
    get(uri: string): Promise<object> {
        return fetch(this.baseUrl + uri, { // Выполняем GET-запрос к указанной конечной точке API
    ...
    post(uri: string, data: object, method: ApiPostMethods = 'POST'): Promise<object> {
        return fetch(this.baseUrl + uri, { // Выполняем POST-запрос к указанной конечной точке API
            ...this.options, // Передаем опции запроса (заголовки и др.)
